          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-2 bg-[#FFFDF9] border border-[#E7DED2] rounded-lg p-1 shadow-sm">
              <button 
                ...
                className={`px-3 py-1 text-xs font-bold rounded ${timeframe === "24h" ? "bg-[#001d36] text-white" : "text-[#74777e]"}`}
              >
                Last 24h
              </button>
              <button 
                onClick={() => setTimeframe("7d")}
                className={`px-3 py-1 text-xs font-bold rounded ${timeframe === "7d" ? "bg-[#001d36] text-white" : "text-[#74777e]"}`}
              >
                Last 7 Days
              </button>
              <button 
                onClick={() => setTimeframe("30d")}
                className={`px-3 py-1 text-xs font-bold rounded ${timeframe === "30d" ? "bg-[#001d36] text-white" : "text-[#74777e]"}`}
              >
                Last 30 Days
              </button>
            </div>

            <button className="bg-[#D98B3A] text-white px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-opacity-90 transition-opacity flex items-center gap-2 shadow-sm">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-8 bg-[#FFFDF9] border border-[#E7DED2] rounded-xl p-6 shadow-sm space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs text-[#74777e]">Aggregate Data</span>
              </div>
              <div className="h-64 bg-[#F7F3EC] rounded-lg border border-[#E7DED2] flex items-end justify-between p-4 gap-2">
                {[45, 60, 35, 90, 120, 80, 95, 110, 70, 85, 105, 130].map((val, idx) => (
            ...
            <div className="lg:col-span-4 bg-[#FFFDF9] border border-[#E7DED2] rounded-xl p-6 shadow-sm space-y-4">
              <h3 className="font-bold text-sm text-[#001d36]">Incident Types Breakdown</h3>
